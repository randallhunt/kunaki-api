...
const apiError = (code: number, text: string): HttpResponse => {
  return HttpResponse.xml(
    builder.build({ Response: { ErrorCode: code, ErrorText: text } })
  )
}

const server = setupServer(
  http.post('https://unittest/XMLService.ASP', async ({ request }) => {
    ...
    if (req.ShippingOptions) {
      ...
      if (!so.State_Province)
        return apiError(4, 'State_Province element is missing')
      if (!so.PostalCode) return apiError(6, 'PostalCode element is missing')
      if (!p || p.length === 0 || !p[0].ProductId)
        return apiError(8, 'ProductId element is missing')
    ...
    if (req.Order) {
      ...
      if (!o.UserId || !o.Password)
        return apiError(12, 'UserId and/or password is not valid')
